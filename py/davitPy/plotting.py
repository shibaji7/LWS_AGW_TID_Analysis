    ...
    **kwArgs
    ...
    def __init__(
        ...
        **kwArgs
        ...
        if paramDict.__contains__("label"):
        ...
        if scale is None:
            if autoScale:
                ...
                if np.min(currentData.data) < 0:
                    ...
                    scale = scMax * np.array([0.0, 1.0])
            else:
                if paramDict.__contains__("range"):
        ...
        lonFull, latFull = (np.array(lonFull) + 360.0) % 360.0, np.array(latFull)

        goodLatLon = np.logical_and(
            np.logical_not(np.isnan(lonFull)), np.logical_not(np.isnan(latFull))
        )
        ...
        tmpmap = Basemap(
            projection="npstere", boundinglat=20, lat_0=90, lon_0=np.mean(goodLonFull)
        )
        ...
        if parallels_ticks is None:
            parallels_ticks = np.arange(-80.0, 81.0, 10.0)

        if meridians_ticks is None:
            meridians_ticks = np.arange(-180.0, 181.0, 20.0)
        ...
        for bm in range(nbeams):
            for rg in range(ngates):
                if goodLatLon[bm, rg] == False:
                    continue
                if np.isnan(data[bm, rg]):
                    continue
                if data[bm, rg] == 0 and not plotZeros:
                    continue
        ...
        if markCell is not None:
            ...
            x1, y1 = m(
                lonFull[beamInx + 0, gateInx + 0], latFull[beamInx + 0, gateInx + 0]
            )
            x2, y2 = m(
                lonFull[beamInx + 1, gateInx + 0], latFull[beamInx + 1, gateInx + 0]
            )
            x3, y3 = m(
                lonFull[beamInx + 1, gateInx + 1], latFull[beamInx + 1, gateInx + 1]
            )
            x4, y4 = m(
                lonFull[beamInx + 0, gateInx + 1], latFull[beamInx + 0, gateInx + 1]
            )
        ...
        if markBeam is not None:
            ...
            for gateInx in range(ngates):
                if goodLatLon[beamInx, gateInx] == False:
                    continue
                x1, y1 = m(
                    lonFull[beamInx + 0, gateInx + 0], latFull[beamInx + 0, gateInx + 0]
                )
        ...
        dataName = currentData.history[
            max(currentData.history.keys())
        ...
        if plot_title:
            if title is None:
                axis.set_title(
                    ...
                    + " - "
                    + dataName
                    + currentData.time[timeInx].strftime("\n%Y %b %d %H%M UT")
                )
            else:
        ...
        if plot_cbar:
            ...
            if currentData.metadata.__contains__("gscat"):
...
class musicRTI(object):
    def __init__(
        ...
        **kwArgs
        ...
        #        if coords is None and metadata.__contains__('coords'):
        #            coords      = metadata['coords']
        #
        if coords not in ["gate", "range"]:
            logging.warning(
                'Coords "%s" not supported for RTI plots.  Using "gate".' % coords
            )
            coords = "gate"
        ...
        for tm in range(nrTimes - 1):
            for rg in range(nrGates - 1):
                ...
                if max_sounding_time is not None:
                    if (
                        currentData.time[tm + 1] - currentData.time[tm + 0]
                    ) > max_sounding_time:
                        continue
        ...
        if (cmap_handling == "matplotlib") or autoScale:
            ...
            colors = "lasse"
            cmap, norm, bounds = utils.genCmap(param, scale, colors=colors)
        ...
        if coords == "gate":
            if secondary_coords:
                if secondary_coords == "range":
                    ...
                    geo_mag = (
                        "Geographic" if currentData.fov.coords == "geo" else "Magnetic"
                    )
                    if metadata["model"] == "IS":
                        axis.set_ylabel(
            ...
            if secondary_coords == "lat":
                ...
                finite_inx = np.where(np.isfinite(currentData.fov.latCenter[beam, :]))[
                    0
                ]
                ...
                axis.set_yticklabels(
                    ytick_str, rotation=90, ma="center"
                )  # Set yticklabels
                # Label y-axis
                geo_mag = (
                    "Geographic" if currentData.fov.coords == "geo" else "Magnetic"
        ...
        if yBoundaryLimits is None:
            if currentData.metadata.__contains__("gateLimits") and coords == "gate":
                yBoundaryLimits = currentData.metadata["gateLimits"]

            if currentData.metadata.__contains__("rangeLimits") and coords == "range":
...
def plotRelativeRanges(dataObj, dataSet="active", time=None, fig=None):
    ...
    absnanmax = np.nanmax(
        np.abs([currentData.fov.relative_x, currentData.fov.relative_y])
    )
    rnd = myround(absnanmax)
    scale = (-rnd, rnd)

    # Determine nanmaximum ranges.
    xRange = np.nanmax(currentData.fov.relative_x) - np.nanmin(
        currentData.fov.relative_x
    )
    yRange = np.nanmax(currentData.fov.relative_y) - np.nanmin(
        currentData.fov.relative_y
    )
    latRange = np.nanmax(currentData.fov.latCenter) - np.nanmin(
        currentData.fov.latCenter
    )
    lonRange = np.nanmax(currentData.fov.lonCenter) - np.nanmin(
        currentData.fov.lonCenter
    )
...
def timeSeriesMultiPlot(
    ...
    if dataObj2 is not None and dataSet2 is None:
        dataSet2 == "active"
    ...
    if xBoundaryLimits is None:
        ...
        if dataSet2 is not None:
            if currentData2.metadata.__contains__("timeLimits"):
    ...
    if ylabel is None and paramDict.__contains__("label"):
        ylabel = paramDict["label"]

    yData1_title = currentData.history[
        max(currentData.history.keys())
    ]  # Label the plot with the current level of data processing
    if title is None:
        title = []
        ...
        title.append(
            currentData.metadata["code"][0].upper()
            + ": "
            + xlim[0].strftime("%Y %b %d %H:%M - ")
            + xlim[1].strftime("%Y %b %d %H:%M")
        )
        title = "\n".join(title)
    ...
    multiPlot(
        ...
        xData2=xData2,
        yData2=yData2,
        yData2_title=yData2_title,
        xBoundaryLimits=xBoundaryLimits,
    )


def spectrumMultiPlot(
    dataObj,
    dataSet="active",
    plotType="real_imag",
    plotBeam=None,
    ...
    if title is None:
        title = []
        title.append(
            "Selected Cells: " + currentData.history[max(currentData.history.keys())]
        )  # Label the plot with the current level of data processing.
        title.append(
            currentData.metadata["code"][0].upper()
            + ": "
...
def plotFullSpectrum(
    ...
    **kwArgs :
    ...
    if xlim is None:
        ...
        posFreqInx = np.where(
            np.logical_and(
                currentData.freqVec >= xlim[0], currentData.freqVec <= xlim[1]
            )
        )[0]
    ...
    if plot_cbar:
        ...
        if currentData.metadata.__contains__("gscat") and cbar_gstext_enable:
            if currentData.metadata["gscat"] == 1:
                cbar.ax.text(
                    0.5,
                    cbar_gstext_offset,
    ...
    for ff in range(npf - 1):
        if (ff % modX) != 0:
            continue
        freqLabel = "%.2f" % (posFreqVec[ff] * 1000.0)
        if posFreqVec[ff] == 0:
            periodLabel = "Inf"
        else:
            periodLabel = "%.0f" % (1.0 / posFreqVec[ff] / 60.0)
    ...
    if plot_title:
        ...
        if md.__contains__("fir_filter"):
            filt = md["fir_filter"]
            if filt[0] is None:
                low = "None"
            else:
                low = "%.2f" % (1000.0 * filt[0])
            if filt[1] is None:
                high = "None"
            else:
                high = "%.2f" % (1000.0 * filt[1])
...
def plotKarr(
    ...
    **kwArgs
    ...
    plotKarrAxis(
        ...
        sig_fontsize=sig_fontsize,
        cbar_gstext_offset=cbar_gstext_offset,
        cbar_gstext_fontsize=cbar_gstext_fontsize,
        **kwArgs
    )
...
def plotKarrDetected(
    dataObj, dataSet="active", fig=None, maxSignals=None, roiPlot=True
):
    """Plot the horizontal wave number array for a pydarn.proc.music.musicArray object.  The kArr must have aready
    been calculated for the chosen data set using pydarn.proc.music.calculateKarr().
    ...
    if roiPlot:
        ...
        if hasattr(currentData, "sigDetect"):
            ...
            tmpList = range(currentData.sigDetect.nrSigs)[
                ::-1
            ]  # Force list to plot backwards so number 1 is on top!
            for signal in currentData.sigDetect.info:
                if maxSignals is not None:
                    if signal["order"] > maxSignals:
                        continue
...
def plotKarrAxis(
    ...
    for y in range(len(ticks)):
        ...
        txt = "\n".join([km_txt, rad_txt])
        newLabels.append(txt)
    axis.set_yticklabels(newLabels, rotation=90.0)
