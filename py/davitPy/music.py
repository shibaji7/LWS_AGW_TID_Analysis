...
def stringify_signal(sig):
    ...
    """
    sigInfo = {}
    if sig.__contains__("order"):
        sigInfo["order"] = (
            "%d" % sig["order"]
        )  # Order of signals by strength as detected by image detection algorithm
    if sig.__contains__("kx"):
        sigInfo["kx"] = "%.5f" % sig["kx"]
    if sig.__contains__("ky"):
        sigInfo["ky"] = "%.5f" % sig["ky"]
    if sig.__contains__("k"):
        sigInfo["k"] = "%.3f" % sig["k"]
    if sig.__contains__("lambda"):
        if np.isinf(sig["lambda"]):
            sigInfo["lambda"] = "inf"
        else:
            sigInfo["lambda"] = "%d" % np.round(sig["lambda"])  # km
    if sig.__contains__("lambda_x"):
        if np.isinf(sig["lambda_x"]):
            sigInfo["lambda_x"] = "inf"
        else:
            sigInfo["lambda_x"] = "%d" % np.round(sig["lambda_x"])  # km
    if sig.__contains__("lambda_y"):
        if np.isinf(sig["lambda_y"]):
            sigInfo["lambda_y"] = "inf"
        else:
            sigInfo["lambda_y"] = "%d" % np.round(sig["lambda_y"])  # km
    if sig.__contains__("azm"):
        sigInfo["azm"] = "%d" % np.round(sig["azm"])  # degrees
    if sig.__contains__("freq"):
        sigInfo["freq"] = "%.2f" % (sig["freq"] * 1000.0)  # mHz
    if sig.__contains__("period"):
        sigInfo["period"] = "%d" % np.round(sig["period"] / 60.0)  # minutes
    if sig.__contains__("vel"):
        if np.isinf(np.round(sig["vel"])):
            sigInfo["vel"] = "Inf"
        else:
            sigInfo["vel"] = "%d" % np.round(sig["vel"])  # km/s
    if sig.__contains__("area"):
        sigInfo["area"] = "%d" % sig["area"]  # Pixels
    if sig.__contains__("max"):
        sigInfo["max"] = (
            "%.4f" % sig["max"]
        )  # Value from kArr in arbitrary units, probably with some normalization
    if sig.__contains__("maxpos"):
        sigInfo["maxpos"] = str(
            sig["maxpos"]
        )  # Index position in kArr of maximum value.
    if sig.__contains__("labelInx"):
        sigInfo["labelInx"] = (
            "%d" % sig["labelInx"]
        )  # Label value from image processing
    if sig.__contains__("serialNr"):
        sigInfo["serialNr"] = (
            "%d" % sig["serialNr"]
        )  # Label value from image processing
    ...
    **metadata
    ...
    def appendHistory(self, comment):
        ...
        self.history[datetime.datetime.now()] = (
            "[" + self.metadata["dataSetName"] + "] " + comment
        )
...
class musicArray(object):
    ...
    def __init__(
        ...
        for myScan in scanReader:
            ...
            for myBeam in myScan.beams:
                # Calculate the field of view if it has not yet been calculated.
                if fov == None:
                    hdw_data = pydarn.read_hdw_file(unit.rad)
                    fov = CalcFov(
                        hdw=hdw_data,
                        elevation=fovElevation,
                        model=fovModel,
                        coords=fovCoords,
                    )
                ...
                # Get the fitData.
                fitDataList = getattr(myBeam, param)
                slist = getattr(myBeam, "slist")
        ...
        for inx in range(len(dataListArray)):
            dataArray[
                int(dataListArray[inx, scanInx]),
                int(dataListArray[inx, beamInx]),
                int(dataListArray[inx, gateInx]),
            ] = dataListArray[inx, dataInx]

        # Make metadata block to hold information about the processing.
        metadata = {}
        metadata["dType"] = "dmap"
        metadata["stid"] = hdw_data.stid
        metadata["name"] = pydarn.SuperDARNRadars.radars[hdw_data.stid].name
        metadata["code"] = hdw_data.abbrev
        metadata["fType"] = "fitacf"
        metadata["cp"] = myBeam.cp
        metadata["channel"] = myBeam.channel
        ...
        setattr(
            self,
            dataSet,
            musicDataObj(timeArray, dataArray, fov=fov, parent=self, comment=comment),
        )
...
def beamInterpolation(
    dataObj,
    dataSet="active",
    newDataSetName="beamInterpolated",
    comment="Beam Linear Interpolation",
):
    ...
    for tt in range(nrTimes):
        for bb in range(nrBeams):
            ...
            if currentData.metadata.__contains__("gateLimits"):
                limits = currentData.metadata["gateLimits"]
                gateInx = np.where(
                    np.logical_and(
                        currentData.fov.gates >= limits[0],
                        currentData.fov.gates <= limits[1],
                    )
                )[0]
    ...
    try:
        if (rangeLimits != None) or (gateLimits != None):
            if (rangeLimits != None) and (gateLimits == None):
                inx = np.where(
                    np.logical_and(
                        currentData.fov.slantRCenter >= rangeLimits[0],
                        currentData.fov.slantRCenter <= rangeLimits[1],
                    )
                )
...
def checkDataQuality(
    dataObj, dataSet="active", max_off_time=10, sTime=None, eTime=None
):
    ...
    time_vec = currentData.time[
        np.logical_and(currentData.time > sTime, currentData.time < eTime)
    ]
...
def applyLimits(
    ...
    try:
        ...
        if (
            currentData.metadata.__contains__("timeLimits") == False
            and currentData.metadata.__contains__("beamLimits") == False
            and currentData.metadata.__contains__("gateLimits") == False
        ):
            return currentData
        ...
        if currentData.metadata.__contains__("gateLimits"):
            ...
            gateInxFull = np.append(
                gateInx, gateInx[-1] + 1
            )  # We need that extra gate since this is the full FOV.
            ...
            rangeLim = (
                np.min(newData.fov.slantRCenter),
                np.max(newData.fov.slantRCenter),
            )
            commentList.append("range [km]: %i,%i" % rangeLim)

            # Remove limiting item from metadata.
            newData.metadata.pop("gateLimits")
            if newData.metadata.__contains__("rangeLimits"):
                newData.metadata.pop("rangeLimits")

        # Apply the beamLimits.
        if currentData.metadata.__contains__("beamLimits"):
            limits = currentData.metadata["beamLimits"]
            beamInx = np.where(
                np.logical_and(
                    currentData.fov.beams >= limits[0],
                    currentData.fov.beams <= limits[1],
                )
            )[0]
            ...
            # This works as long as we look at only consecutive gates.  If we ever do something where we are not looking at consecutive gates
            # (typically for computational speed reasons), we will have to do something else.
            beamInxFull = np.append(
                beamInx, beamInx[-1] + 1
            )  # We need that extra beam since this is the full FOV.
        ...
        if currentData.metadata.__contains__("timeLimits"):
            limits = currentData.metadata["timeLimits"]
            timeInx = np.where(
                np.logical_and(
                    currentData.time >= limits[0], currentData.time <= limits[1]
                )
            )[0]
            ...
            commentList.append(
                "time: "
                + limits[0].strftime("%Y-%m-%d/%H:%M,")
                + limits[1].strftime("%Y-%m-%d/%H:%M")
            )
            # Remove limiting item from metadata.
        ...
        traceback.print_exc()
        if hasattr(dataObj, newDataSetName):
            delattr(dataObj, newDataSetName)
        print("Warning! Limits not applied.")
        return currentData


def determineRelativePosition(dataObj, dataSet="active", altitude=250.0):
...
def timeInterpolation(
    dataObj,
    dataSet="active",
    newDataSetName="timeInterpolated",
    comment="Time Linear Interpolation",
    timeRes=10,
    newTimeVec=None,
):
    ...
    sTime = datetime.datetime(
        sTime.year, sTime.month, sTime.day, sTime.hour, sTime.minute
    )  # Make start time a round time.
    ...
    good = np.where(
        np.logical_and(
            newTimeVec > min(currentData.time), newTimeVec < max(currentData.time)
        )
    )
...
def filterTimes(sTime, eTime, timeRes, numTaps):
    ...
    """
    td = datetime.timedelta(seconds=(numTaps * timeRes / 2.0))
    newSTime = sTime - td
    newETime = eTime + td
...
class filter(object):
    ...
    def __init__(
        ...
        md = sigObj.metadata
        if cutoff_high == None:
            if md.__contains__("filter_cutoff_high"):
                cutoff_high = md["filter_cutoff_high"]

        if cutoff_low == None:
            if md.__contains__("filter_cutoff_low"):
                cutoff_low = md["filter_cutoff_low"]

        if numtaps == None:
            if md.__contains__("filter_numtaps"):
        ...
        if cutoff_high != None and cutoff_low != None:
            d = -(lp + hp)
            d[int(numtaps / 2)] = d[int(numtaps / 2)] + 1
            d = -1.0 * d  # Needed to correct 180 deg phase shift.
    ...
    def plotTransferFunction(
        ...
        if ymin_mag is not None:
            axis.set_ylim(ymin=ymin_mag)
        if ymax_mag is not None:
            axis.set_ylim(ymax=ymax_mag)
        ...
        if ymin_phase is not None:
            axis.set_ylim(ymin=ymin_phase)
        if ymax_phase is not None:
            axis.set_ylim(ymax=ymax_phase)
    ...
    def filter(self, dataObj, dataSet="active", newDataSetName="filtered"):
        ...
        shift = np.int32(-np.floor(len(self.ir) / 2.0))
        ...
        if hasattr(newsigobj, "metadata"):
            ...
            for key in delMeta:
                if newsigobj.metadata.__contains__(key):
...
def detrend(
    dataObj, dataSet="active", newDataSetName="detrended", comment=None, type="linear"
):
...
def windowData(
    dataObj, dataSet="active", newDataSetName="windowed", comment=None, window="hann"
):
...
def calculateFFT(dataObj, dataSet="active", comment=None):
    ...
    for bm in range(nrBeams):
        for rg in range(nrGates):
            newDataArr[:, bm, rg] = fftpack.fftshift(
                fftpack.fft(currentData.data[:, bm, rg])
            ) / np.size(currentData.data[:, bm, rg])
    ...
    data = np.abs(
        currentData.spectrum[posFreqInx, :, :]
    )  # Use the magnitude of the positive frequency data.

    # Average Power Spectral Density
    avg_psd = np.zeros(npf)
    for x in range(npf):
        avg_psd[x] = np.mean(data[x, :, :])
...
def calculateDlm(dataObj, dataSet="active", comment=None):
    ...
    for ll in range(nCells):
        ...
        currentData.llLookupTable[:, ll] = [
            ll,
            currentData.fov.beams[llAI[0]],
            currentData.fov.gates[llAI[1]],
            ns_dist,
            ew_dist,
        ]
...
def simulator(
    ...
    if keepLocalRange == True:
        ...
        xvec = np.linspace(-xRange / 2.0, xRange / 2.0, nx)
        yvec = np.linspace(-yRange / 2.0, yRange / 2.0, ny)
    ...
    if sigs == None:
        ...
        sigs = []
        #           (amp,    kx,      ky,      f, phi, dcOffset)
        sigs.append((5, 0.01, -0.010, 0.0004, 0, 5.0))
        sigs.append((5, 0.022, -0.023, 0.0004, 0, 5.0))
    ...
    for step in range(nSteps):
        ...
        for kk in range(len(sigs)):
            ...
            temp = (
                amp * np.cos(kx * xgrid + ky * ygrid - 2.0 * np.pi * f * t + phi) + dc
            )
    ...
    rgDist = (
        1.0 / (sigma * np.sqrt(2.0 * np.pi)) * np.exp(-0.5 * ((xx - mu) / sigma) ** 2)
    )
...
def detectSignals(dataObj, dataSet="active", threshold=0.35, neighborhood=(10, 10)):
    ...
    local_maxi = peak_local_max(
        distance, footprint=np.ones(neighborhood), indices=False
    )
    ...
    for x in range(labels.max()):
        ...
        info["period"] = 1.0 / currentData.dominantFreq
        info["vel"] = (2.0 * np.pi / info["k"]) * info["freq"] * 1000.0
