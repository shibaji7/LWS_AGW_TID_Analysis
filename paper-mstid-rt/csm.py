...
def visualize_polyline_rays(rays_data):
    ...
    available_colors = [
        cesiumpy.color.DEEPSKYBLUE,
        cesiumpy.color.ORANGERED,
        cesiumpy.color.LIMEGREEN,
        cesiumpy.color.GOLD,
        cesiumpy.color.DARKVIOLET,
        cesiumpy.color.CYAN,
    ...
    for ray_index, single_ray_coords in enumerate(rays_data):
        ...
        max_alt_in_ray = (
            max(p[2] for p in single_ray_coords if len(p) > 2)
            if single_ray_coords
            else 0
        )
        ...
        for point_idx, (lon, lat, alt_km) in enumerate(single_ray_coords):
            ...
            if point_idx == 0:
                marker_name, marker_color, marker_pixel_size, create_marker_flag = (
                    f"Ray {ray_index+1} Start",
                    current_ray_color,
                    10,
                    True,
                )
                ...
                marker_name, marker_color, marker_pixel_size, create_marker_flag = (
                    f"Ray {ray_index+1} Bounce",
                    cesiumpy.color.WHITE,
                    8,
                    True,
                )
            elif alt_km == max_alt_in_ray and alt_km > 0:
                marker_name, marker_color, marker_pixel_size, create_marker_flag = (
                    f"Ray {ray_index+1} Apex",
            ...
            if create_marker_flag:
                point_graphics_options = {
                    ...
                    "outlineColor": marker_outline_color,
                ...
                try:
                    marker_entity = cesiumpy.entities.entity.Entity(
                        ...
                        name=marker_name,
                        description=point_description_html,
                        point=point_graphics_options,
                    )
                    viewer.entities.add(marker_entity)
                except (
                    AttributeError,
                    TypeError,
                ):  # Catch if .Entity is not there or .entity is a module
                    try:
                        marker_entity = (
                            cesiumpy.entities.entity.entity(  # Try lowercase class name
                                position=cartesian_point,
                                name=marker_name,
                                description=point_description_html,
                                point=point_graphics_options,
                        ...
                        print(
                            f"CRITICAL: Failed to create Entity. Error: {e_innermost}"
                        )
        ...
        if cesium_points_for_current_polyline:
            ...
            try:
                current_polyline = cesiumpy.entities.polyline.Polyline(
                    positions=cesium_points_for_current_polyline,
                    material=current_ray_color.withAlpha(0.75),
                    width=3,
                    name=f"Polyline Ray {ray_index + 1}",
                    description=polyline_description_html,
                )
                viewer.entities.add(current_polyline)
            except (AttributeError, TypeError):
                try:
                    current_polyline = (
                        cesiumpy.entities.polyline.polyline(  # Try lowercase class name
                            positions=cesium_points_for_current_polyline,
                            material=current_ray_color.withAlpha(0.75),
                            width=3,
                            name=f"Polyline Ray {ray_index + 1}",
                    ...
                    print(
                        f"CRITICAL: Failed to create Polyline. Error: {e_innermost_poly}"
                    )
    ...
    if all_lons and all_lats:
        ...
        lon_buffer, lat_buffer = (
            abs(max_lon - min_lon) * 0.15 + 2.0,
            abs(max_lat - min_lat) * 0.15 + 2.0,
        )
        west, south = max(-180.0, min_lon - lon_buffer), max(
            -90.0, min_lat - lat_buffer
        )
        ...
        try:
            ...
            print(
                "WARNING: viewer.fly_home() not found. Camera may not reset if no data."
            )
