...
% for i=1:num_elevs
%	X = [ray_path_data(i).height; ray_path_data(i).ground_range; ray_path_data(i).group_range; 
%        ray_path_data(i).phase_path; ray_path_data(i).geometric_distance; ray_path_data(i).refractive_index;
%        ray_path_data(i).electron_density].';
%	T = array2table(X);
%	T.Properties.VariableNames(1:7) = {'height','grange','grprange','phpath','srange','refi','eden'};
%    if elevs(i) < 10
%        fn = strrep(fname, '<elv>', join(['0', num2str(elevs(i), '%2.2f')]));
%    else
%        fn = strrep(fname, '<elv>', num2str(elevs(i), '%2.2f'));
%    end
%	writetable(T, fn);
%end
