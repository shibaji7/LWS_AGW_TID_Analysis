...
def get_point_at_distance(lat, lon, d, bearing, R=6371):
    ...
    lon_d = lon + atan2(
        sin(a) * sin(d / R) * cos(lat), cos(d / R) - sin(lat) * sin(lat_d)
    )
...
class RayTrace3D(object):
    ...
    def _estimate_bearing_(self):
        ...
        lat_d, lon_d = get_point_at_distance(
            lat_start, lon_start, self.cfg.distance_travel_km, bearing
        )
        logger.info(
            f"Difference in degrees: {np.round(lat_start-lat_d,2)}, {np.round(lon_d-lon_start,2)}"
        )
        num_lat, num_lon = (
...
def execute_iri3D_simulations(args):
    ...
    for i in range(args.time_steps_min):
        tid_prop = dict(lamb_x=1000, v_x=0.1, a_lim=[0.1, 1], t=i, source="center")
