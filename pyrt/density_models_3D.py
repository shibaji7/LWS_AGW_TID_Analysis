...
class Iono3D(object):
    ...
    def create_grids(self):
        lats = np.arange(
            ...
            self.bearing_obj["lat_start"]
            + self.bearing_obj["lat_inc"] * int(self.bearing_obj["num_lat"]),
        ...
        lons = np.arange(
            ...
            self.bearing_obj["lon_start"]
            + self.bearing_obj["lon_inc"] * self.bearing_obj["num_lon"],
        ...
        alts = np.arange(
            ...
            self.bearing_obj["ht_start"]
            + self.bearing_obj["ht_inc"] * self.bearing_obj["num_ht"],
            self.bearing_obj["ht_inc"],
        )
        self.alts = alts[: int(self.bearing_obj["num_ht"])]
        self.edens = np.zeros((len(self.lats), len(self.lons), len(self.alts))) * np.nan
    ...
    def create_TIDs(self):
        ...
        A = np.sin(
            2
            * np.pi
            * (dist - self.tid_prop["v_x"] * self.tid_prop["t"])
            / self.tid_prop["lamb_x"]
        )
        # Rescale to 0-1
