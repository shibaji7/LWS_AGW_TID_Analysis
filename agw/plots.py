...
class PlotKarr(object):
    ...
    def __init__(
        self, Karr, kxVec, kyVec,
        maxSignals=None, sig_fontsize=24,
        plot_title=True, cbar_ticks=None,
        cbar_shrink=1.0, cbar_fraction=0.15,
    ...
    def draw(self):
        ...
        if plot_colorbar:
            cbar = self.fig.colorbar(pcoll,orientation='vertical',shrink=cbar_shrink,fraction=cbar_fraction,pad=cbar_pad)
...
#                     cbar.ax.text(0.5,cbar_gstext_offset,'Ground\nscat\nonly',ha='center',fontsize=cbar_gstext_fontsize)
        ...
        for y in range(len(ticks)):
            ...
            txt = '\n'.join([km_txt,rad_txt])
            newLabels.append(txt)
        self.axis.set_yticklabels(newLabels,rotation=90.)
        self.axis.set_ylabel(u'ky [rad]\n$\lambda$ [km]',va='center',labelpad=y_labelpad)
        return
