    ...
    **metadata
    ...
    def samplePeriod(self,timeVec=None):
        ...
        if len(diffs_unq) == 1:
            ...
            md          = self.metadata
            warn        = 'WARNING'
            if md.__contains__('title'): warn = ' '.join([warn,'FOR','"'+md['title']+'"'])
...
class musicArray(object):
    ...
    """
    def __init__(self,unit,sTime=None,eTime=None,param='p_l',gscat=1,
...
#            myScan = pydarn.sdio.radDataRead.radDataReadScan(myPtr)
            ...
            for myBeam in myScan.beams:
                ...
                if fov == None:
                    ...
                    fov       = CalcFov(hdw=hdw_data, elevation=fovElevation,model=fovModel,coords=fovCoords)
...
def applyLimits(dataObj,dataSet='active',rangeLimits=None,gateLimits=None,timeLimits=None,newDataSetName='limitsApplied',comment=None):
    ...
    try:
        ...
        if (currentData.metadata.__contains__('timeLimits') == False and 
            currentData.metadata.__contains__('beamLimits') == False and 
            currentData.metadata.__contains__('gateLimits') == False):
            return currentData
        ...
        if currentData.metadata.__contains__('gateLimits'):
            ...
            if newData.metadata.__contains__('rangeLimits'): newData.metadata.pop('rangeLimits')
